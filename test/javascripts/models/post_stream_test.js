...
var buildStagedPost = function(user) {
  return Discourse.Post.create({
    raw: 'hello world this is my new post',
    cooked: '<p>hello world this is my new post</p>',
    reply_count: 0,
...
test('appending posts', function() {
  ...
  postStream.get('posts.length', 0, "it has no posts initially");
  ...
  postStream.get('posts.length', 1, "it has one post in the stream");
  ...
  ok(!postStream.get('lastPostLoaded'), "the last post is not loaded");
  postStream.get('posts.length', 2, "it has two posts in the stream");

  postStream.appendPost(Discourse.Post.create({id: 4, post_number: 4}));
  postStream.get('posts.length', 2, "it will not add the same post twice");
...
test("storePost", function() {
  ...
  var post = Discourse.Post.create({id: 1, post_number: 1});
  ...
  var dupePost = Discourse.Post.create({id: 1, post_number: 1});
  var storedDupe = postStream.storePost(post);
...
test("staging and undoing a new post", function() {
  ...
  postStream.set('highest_post_number', 1);

  var user = Discourse.User.create({username: 'eviltrout', name: 'eviltrout', id: 321});
  var stagedPost = buildStagedPost(user);
  ...
  equal(postStream.get('highest_post_number'), 2, "it updates the highest_post_number");
  ...
  equal(postStream.get('highest_post_number'), 1, "it reverts the highest_post_number");
...
test("staging and committing a post", function() {
  ...
  // Stage the new post in the stream
  postStream.stagePost(stagedPost, user);
  stagedPost.setProperties({ id: 1234, cooked: "different cooked value" });
  ...
  equal(found.get('cooked'), 'different cooked value', 'it also updated the value in the stream');
