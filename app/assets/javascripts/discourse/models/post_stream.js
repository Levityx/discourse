...
Discourse.PostStream = Em.Object.extend({
  ...
  **/
  appendMore: function() {
    ...
    var rejectedPromise = Ember.Deferred.create(function(p) { p.reject(); });
  ...
  **/
  stagePost: function(post, user) {
    ...
    this.set('highest_post_number', (this.get('highest_post_number') || 0) + 1);
  ...
  **/
  commitPost: function(post) {
    ...
    this.get('stream').push(post.get('id'));
  },
  ...
  **/
  undoPost: function(post) {
    ...
    this.setProperties({
      highest_post_number: (this.get('highest_post_number') || 0) - 1,
      'topic.posts_count': (this.get('topic.posts_count') || 0) - 1
    });
  },
  ...
  **/
  appendPost: function(post) {
    this.get('posts').pushObject(this.storePost(post));
    return post;
  },
  ...
  **/
  storePost: function(post) {
    ...
    var existing = postIdentityMap.get(post.get('id'));
  ...
  **/
  listUnloadedIds: function(postIds) {
    ...
    postIds.forEach(function(p) {
      if (!postIdentityMap.has(p)) { unloaded.push(p); }
    });
    return unloaded;
  },
